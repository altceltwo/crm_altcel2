...
<header class="page-header">
    ...
    <div class="right-wrapper pull-right">
        <ol class="breadcrumbs">
            ...
            <li>
                <a href="{{route('preactivations.index')}}">Volver</a>
            </li>
        </ol>
                    ...
                    <div class="col-md-6">
                        <div class="bill-data text-right">
                            <p class="mb-none">
                                ...
                                <span class="text-dark">{{$activation->date_activation}}</span>
                            </p>
                            <p class="mb-none">
                                <span class="text-dark text-semibold">Añadido por:</span>
                                <span class="text-dark">{{$user->name.' '.$user->lastname}}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <form class="form-horizontal form-bordered" >
                    <div class="form-group"  style="padding-right: 1rem; padding-left: 1rem;">
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="hidden" id="activation_id" value="{{$activation->id}}">
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    $('#confirmChangeRate').click(function(){
        let rate = $('#rate').val();
        let offer = $('#rate option:selected').attr('data-offer-id');
           ...
           return false;
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{route('changeRatePreactivate')}}",
                        method: "POST",
                        data: {_token:token, rate_id:rate, offer_id:offer, activation_id:activation_id},
    ...
    $('#rollback').click(function(){
        let activation_id = $('#activation_id').val();
        let token = $('meta[name="csrf-token"]').attr('content');
        let url = "{{route('rollbackPreactivate',['activation'=>'temp'])}}";
        url = url.replace('temp',activation_id);
        
        Swal.fire({
            title: '¿Está seguro de revertir la preactivación?',
            text: 'Los datos de este registro serán eliminados permanentemente.',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#47a447',
            cancelButtonColor: '#d33',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        if(response == 1){
                            location.href = "{{route('preactivations.index')}}";
                        }else{
                            Swal.fire({
                                icon: 'error',
                                title: 'Ha ocurrido un error, consulte a soporte.',
    ...
    $('#activate').click(function(){
        let activation_id = $('#activation_id').val();
        let token = $('meta[name="csrf-token"]').attr('content');
        let url = "{{route('executeActivation',['activation'=>'temp'])}}";
        url = url.replace('temp',activation_id);
                ...
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        if(response.http_code == 1){
                            ...
                            setTimeout(function(){ location.href = "{{route('preactivations.index')}}"; }, 1600);
